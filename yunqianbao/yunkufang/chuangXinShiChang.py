...
class ChuangXinShiChang(TaskSet):
    ...
    def cxsc_list(self,token,header):
        ...
        with self.client.post(list_url,data = list_data, headers = header,name = list_urlName+list_url,verify = False,allow_redirects=False,catch_response=True,timeout = 30) as response:
            result = json.loads(response.text)
            if "status" in result and result["status"] == 200:
                response.success()
                return result
            else:
                response.failure("报错url==={}-{} ，参数==={} ，报错原因==={}".format(list_urlName,list_url,list_data,list))
    ...
    def quRenGou(self,detaill_obj,token,header):
        ...
        if pay_status == 0:
            ...
            with self.client.post(qrg_url,data = qrg_data, headers = header,name = qrg_urlName+qrg_url,verify = False,allow_redirects=False,catch_response=True,timeout = 30) as response:
    ...
    def renGou(self,qurengou,detaill_obj_id,token,header):
        ...
        with self.client.post(rg_url,data = rg_data, headers = header,name = rg_urlName+rg_url,verify = False,allow_redirects=False,catch_response=True,timeout=30) as response:
