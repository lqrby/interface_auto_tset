...
class YunQianBaoMan(TaskSet):
    ...
    def userShiMing(self):
        ...
        if taskdata["type"] == 5:
            ...
            print("{}用户状态==={}===待审核".format(self.login_res["mobile"],taskdata["type"]))
        elif taskdata["type"] == 1:
            print("{}用户状态==={}===未实名,现在去提交审核资料".format(self.login_res["mobile"],taskdata["type"]))
            self.publicData.shiMing(self.apikey,self.header,self.login_res,self.sfz_path) #实名认证
        else:
            print("{}用户状态?====={}".format(self.login_res["mobile"],taskdata["type"]))
